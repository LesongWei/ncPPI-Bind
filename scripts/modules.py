...
class MultiHeadAttentionSequence(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        Q = self.W_Q(q).view([batch, len_q, self.n_head, self.d_k]).contiguous()
        K = self.W_K(k).view([batch, len_k, self.n_head, self.d_k]).contiguous()
        V = self.W_V(v).view([batch, len_v, self.n_head, self.d_v]).contiguous()
        ...
        output = output.transpose(1, 2).reshape([batch, len_q, self.d_v*self.n_head]).contiguous()
...
class MultiHeadAttentionReciprocal(nn.Module):
    ...
    def forward(self, q, k, v, v_2):
        ...
        K = self.W_K(k).view([batch, len_k, self.n_head, self.d_k]).contiguous()
        V = self.W_V(v).view([batch, len_v, self.n_head, self.d_v]).contiguous()
        V_2 = self.W_V_2(v_2).view([batch, len_v_2, self.n_head, self.d_v]).contiguous()
        ...
        K = K.transpose(1, 2).transpose(2, 3).contiguous()
        V = V.transpose(1, 2).contiguous()
        V_2 = V_2.transpose(1,2).contiguous()
        ...
        attention_2 = attention.transpose(-2, -1).contiguous()
        ...
        output_2 = output_2.transpose(1, 2).reshape([batch, len_k, self.d_v*self.n_head]).contiguous()
